...
import { Book, AlertTriangle, Lightbulb, Volume2 } from 'lucide-react';
                        ...
                        <div className="space-y-3">
                            {item.examples.map((ex, idx) => (
                                <div key={idx} className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4 bg-white p-3 rounded-lg border border-indigo-100 shadow-sm">
                                    <button 
                                        onClick={() => playTTS(ex.french)}
                                        className="p-1.5 text-indigo-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-full transition-colors flex-shrink-0"
                                    >
                                        <Volume2 size={16} />
                                    </button>
