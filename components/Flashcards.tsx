...
const Flashcards: React.FC<FlashcardsProps> = ({ words }) => {
  ...
  const playAudio = async (e: React.MouseEvent, text: string) => {
    ...
    try {
      ...
      if (!resp.ok) throw new Error("生成语音失败");
    ...
    if (currentIndex < filteredWords.length - 1) {
      ...
      alert(
        `本组练习完成！\n正确: ${sessionStats.correct + (correct ? 1 : 0)}\n需复习: ${
          sessionStats.wrong + (correct ? 0 : 1)
        }`
      );
      setViewMode("list");
    }
                  ...
                  <div className="flex justify-between items-start mb-2">
                    ...
                    {status && (
                      <div className="flex items-center gap-1 text-[10px] font-bold text-orange-600 bg-orange-50 px-2 py-0.5 rounded-full">
                        <AlertCircle size={10} /> 待复习 ({status.masteryLevel}/5)
                      </div>
                    )}
                  </div>
      ...
      >
        ...
        >
          ...
          >
            ...
            <h2 className="text-5xl md:text-6xl font-bold text-slate-800 text-center mb-6 break-words max-w-full">
              {currentWord.french}
            </h2>
            <button
              onClick={(e) => playAudio(e, currentWord.french)}
              className="p-3 bg-indigo-50 text-indigo-600 rounded-full hover:bg-indigo-100 transition-colors mb-4"
            >
              <Volume2 size={24} />
            </button>
          ...
          >
            ...
            <p className="text-indigo-300 text-xl font-mono mb-6 border-b border-indigo-500/30 pb-4 inline-block">
              {currentWord.ipa}
            </p>
